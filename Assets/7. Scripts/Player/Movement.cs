...
public class Movement : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		Vector3 targetRotation = GetTargetRotation();
		
		if (targetVelocity.sqrMagnitude > 0)
		{
			Rotating(targetVelocity);
	...
	void Update()
	{
		AnimationMovement (GetTargetVelocity().magnitude);
	}
	
	private void Rotating (Vector3 targetVelocity)
	{
		if (targetVelocity.sqrMagnitude != 0)
		{
	...
	private Vector3 GetTargetRotation()
	{
		...
		Vector3 targetVelocity = 
			(camDirection * _avatar.controller.Axis (Axis.RightVertical)) + 
				(camRight * _avatar.controller.Axis (Axis.RightHorizontal));
